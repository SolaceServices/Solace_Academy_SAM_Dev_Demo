...
COURSE_ROOT="${1:-/workspaces/Solace_Academy_SAM_Dev_Demo/600-Troubleshooting}"
...
# ----------------------------
# EMAIL SERVICE SETUP (Module 600 only)
# ----------------------------
...
# ----------------------------
# Add Email Tool to IncidentResponseAgent (Module 600 only)
# ----------------------------
INCIDENT_AGENT="$SAM_DIR/configs/agents/incident_response_agent_agent.yaml"

if [ -f "$INCIDENT_AGENT" ]; then
  python3 - "$INCIDENT_AGENT" <<'PYTHON_EMAIL'
import sys
...
if 'send_alert_email' not in content:
    email_instructions = """        When creating high-severity incidents (severity='high'):
        ...
        Title: Critical Stock Shortage - Laptop Pro 15
...
if 'email_tool' not in content:
    ...
    content = content.replace('          tool_name: list_artifacts', '          tool_name: list_artifacts' + email_tool, 1)

with open(agent_file, 'w') as f:
    f.write(content)
PYTHON_EMAIL

  echo "✅ Email tool added to IncidentResponseAgent"
else
  echo "⚠️  Warning: IncidentResponseAgent config not found at $INCIDENT_AGENT"
fi

# ----------------------------
# Enable Scheduler Service in WebUI Gateway (Module 600 only)
# ----------------------------
...
# ----------------------------
# Generate Morning Briefing Workflow (Module 600 only)
# ----------------------------
