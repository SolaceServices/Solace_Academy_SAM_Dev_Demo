...
def scene3(t):
    ...
    if phase == 'flat' and t > 33: 
        put(buffer, truck_top+1, tx, '|  ACME LOGISTICS  |  |>|  \\', Y())
    else: 
        put(buffer, truck_top+1, tx, '|  ACME LOGISTICS  |  |>|o \\', Y()) 
    ...
    if phase == 'flat':
        ...
        if t > 28:
            ...
            put(buffer, box_top + 6, box_col, ('| !! SHIPMENT DELAY INCIDENT CREATED !!').ljust(box_w+1) + '|', R())
            put(buffer, box_top + 7, box_col, ('| SHIP-2026-0048  flat tire  Hwy 101').ljust(box_w+1) + '|', R())
            put(buffer, box_top + 8, box_col, ('| INC created  ETA +2h - team notified').ljust(box_w+1) + '|', R())
    ...
    if phase == 'repaired':
        ...
        put(buffer, box_top + 6, box_col, ('| >> Tire replaced! Back on the road.').ljust(box_w+1) + '|', G())
        put(buffer, box_top + 7, box_col, ('| SHIP-2026-0048  in_transit  ETA updated').ljust(box_w+1) + '|', G())
        put(buffer, box_top + 8, box_col, ('| Customer notified via SMS + email').ljust(box_w+1) + '|', G())
